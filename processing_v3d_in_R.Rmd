---
title: "Processing V3D Data in R"
author: "Mac Prible"
date: "`r Sys.Date()`"
output: html_document
params:
  subject: "s1"
---

```{r setup, include=FALSE}


# knitr::opts_chunk$set(echo = TRUE)
source("tidy_v3d.R")

# Set up variables
raw_data_directory <- "C:\\Users\\Mac Prible\\OneDrive - The University of Texas at Austin\\research\\PDSV\\data\\PDVS_2024\\v3d\\raw_v3d_output"
tidy_data_directory <- "C:\\Users\\Mac Prible\\OneDrive - The University of Texas at Austin\\research\\PDSV\\data\\PDVS_2024\\v3d\\tidy_output"
subject_reports <- "C:\\Users\\Mac Prible\\OneDrive - The University of Texas at Austin\\research\\PDSV\\data\\PDVS_2024\\v3d\\subject_reports"

# Define function to process single file with side information
process_side_file <- function(data_directory, subject_id, side) {
  file_name <- str_glue("{subject_id}_{side}.tsv")
  
  tidy_data <- tsv_to_tidy(data_directory = data_directory, 
                           file_name = file_name) %>%
                  tidy_by_period() 
  
  return(tidy_data)
  
}

# Process both sides and combine
process_both_sides <- function(data_directory, subject_id) {
  sides <- c("left", "right")
  
  combined <- map_df(sides, 
                     ~process_side_file(data_directory = data_directory,
                                        subject_id = subject_id,
                                        side = .x)) 
  
  return(combined)
}


# tidy up subject's raw data
combined_data <- process_both_sides(raw_data_directory, params$subject)

```


# Assignment of Periods


```{r}
combined_data %>%  
  filter(str_detect(variable, "Belt")) %>% 
  filter(axis=="X") %>% 
  pivot_wider(names_from = variable, values_from = value) %>% 
  mutate(speed_difference = IPSI_BeltSpeed- CONTRA_BeltSpeed,
         abs_speed_difference = abs(speed_difference)) %>%  
  ungroup() %>% 
  group_by(subject, order,condition,start_stop, stance_side,axis, step, period) %>% 
  summarize(max_speed_diff = max(abs_speed_difference)) %>% 
  ggplot(aes(x= step, y = max_speed_diff, color=period))+
  geom_point()+
  facet_grid(cols = vars(stance_side,start_stop), rows=vars(condition))

```


# Belt Speed Across Condions by Period
```{r, fig.width=15, fig.height=10}
combined_data %>%
  filter(variable == "IPSI_BeltSpeed") %>%
  filter(axis == "X") %>%
  ggplot(aes(x = normalized_time, y=value, group=step, color=period)) +
  geom_path(alpha = 0.7) +
  facet_grid(
    cols = vars(period, stance_side), 
    rows = vars(condition)
  ) +
  scale_x_continuous(labels=NULL)+
  labs(
    x = "Normalized Stance Time (0-100%)",
    y = "Belt Speed (m/s)",
    title = "Belt Speed Across Conditions",
    subtitle = "Faceted by period and stance side",
    color = "Condition"
  )+
  theme(legend.position = "bottom")

```


# Ankle Torque Across Conditions by Period

```{r, fig.width=15, fig.height=10}

combined_data %>%
  filter(variable == "IPSI_ANKLE_MOMENT") %>%
  filter(axis == "X") %>%
  # filter(period %in% c("Late Baseline", "Early Post Adaptation")) %>% 
  ggplot(aes(x = normalized_time, y=value, group=step, color=period)) +
  geom_path(alpha = 0.7) +
  facet_grid(
    cols = vars(period, stance_side), 
    rows = vars(condition))+
  
  labs(
    x = "Normalized Time",
    y = "Belt Speed (m/s)",
    title = "Ankle Torque Across Conditions",
    subtitle = "Faceted by period and stance side",
    color = "Condition"
  )+
  theme(legend.position = "bottom")
```


# Step Length

```{r}

# Calculate step length

combined_data%>% 
  filter(normalized_time==1) %>% 
  filter(axis == "Y") %>% 
  filter(grepl("*_HEEL", variable)) %>% 
  pivot_wider(names_from = variable, values_from = value) %>% 
  mutate(step_length = abs(IPSI_HEEL-CONTRA_HEEL)) %>% 
  group_by(subject, order, condition, start_stop, stance_side, period) %>% 
  summarize(step_length = mean(step_length)) %>% 
  ungroup() %>% 
  pivot_wider(
    names_from = stance_side, 
    values_from = step_length,
    names_glue = "{stance_side}_step_length"  # This adds the suffix to each name
  ) %>% 
  mutate(SLR = left_step_length/right_step_length) %>% 
  ggplot(aes(x=period, y = SLR))+
  geom_point()+
  facet_grid(rows=vars(condition))

```

# Time

```{r}
  
combined_data %>% 
  filter(axis == "X") %>% 
  filter(variable=="TIME") %>%
  pivot_wider(names_from = variable,values_from = value) %>%
  ungroup() %>% 
  group_by(subject, order, condition, start_stop, stance_side, period, normalized_time) %>% 
  summarise(TIME = mean(TIME)) %>%
  ungroup() %>% 
  group_by(subject, order, condition, start_stop, stance_side, period) %>% 
  summarize(min_time = min(TIME),
            max_time = max(TIME)) %>% 
  mutate(stance_time = max_time-min_time) 
  
  ggplot(aes(x=normalized_time,y=TIME))+
  geom_line()+
  facet_grid(cols=vars(start_stop,period), rows=vars(condition))
```

# PF

```{r}




```


