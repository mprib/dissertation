...
ggsave(file.path(figure_directory,"Bilateral PDSV speeds.svg"))
```
...
# Moment of Support Across Stance Phase

## Plot of Characteristic Curves
```{r, fig.width=6, fig.height=8}

# comparison <- pre_late_periods
comparison <- c("Late Baseline", "Late Adaptation")

comparison = pre_post_periods

support_moment_across_stance <- mean_across_stance %>% 
  filter(period %in% comparison) %>% 
  filter(condition %in% names(BIL_CONDITION_LABELS)) %>%  
  filter(variable %in% c("IPSI_ANKLE_MOMENT", "IPSI_KNEE_MOMENT", "IPSI_HIP_MOMENT") & axis == "X") %>% 
  mutate(variable = case_when(variable =="IPSI_ANKLE_MOMENT" ~ "Ankle",
                              variable =="IPSI_KNEE_MOMENT" ~ "Knee",
                              variable =="IPSI_HIP_MOMENT" ~ "Hip")) %>% 
  pivot_wider(names_from = variable, values_from = value) %>% 
  mutate(Hip = -Hip,  # fix sign for moment of support
         Ankle = -Ankle,  # fix sign for moment of support
         Support = Hip + Knee + Ankle) %>% 
  pivot_longer(cols=c(Hip, Knee, Ankle, Support), names_to = "variable") %>% 
  mutate(variable = factor(variable, levels=c("Support", "Hip","Knee", "Ankle")))

support_moment_across_stance %>% 
  filter(variable!="Support") %>%  # simplify plot
  mutate(condition = BIL_CONDITION_LABELS[condition]) %>% 
  group_by(period, condition, variable, normalized_time) %>% 
  summarize(
    joint_torque = mean(value),
    se = sd(value)/sqrt(n()),  # Standard error
    ci = 1.96 * se,            # 95% confidence interval
    upper = joint_torque + ci,
    lower = joint_torque - ci,
    .groups = 'drop'
  ) %>% 
  ggplot(aes(x = normalized_time, color = condition, linetype = period)) +
  geom_ribbon(aes(
  ...
  geom_line(aes(y = joint_torque), size = 1) +
  geom_hline(yintercept = 0) +
  scale_x_continuous(labels = NULL) +
...
ggsave(file.path(figure_directory,"bil_torque_over_time.svg"))
...
moment_by_type <- support_moment_across_stance %>% 
  group_by(subject,variable, condition, period, normalized_time) %>%
  summarize(value = mean(value)) %>% 
  mutate(moment_type = case_when(# Knee phases
                           variable == "Knee" & value > 0~ "Extension",
                           variable == "Knee" & value < 0~ "Flexion",
                           
                           # Ankle phases (based on moment sign)
                           variable == "Ankle" & value > 0 ~ "Extension", # or Plantarflexion
                           variable == "Ankle" & value < 0 ~ "Flexion", # or Dorsiflexion
                           
                           # Hip phases (based on moment sign)
                           variable == "Hip" & value > 0 ~ "Extension",
                           variable == "Hip" & value < 0 ~ "Flexion"
                           )
  ) %>% 
  filter(variable!="Support") 
...
moment_by_type %>% 
  ...
  facet_grid(cols = vars(period, condition), 
             rows = vars(variable, moment_type),
             scales = "free_y") +
  scale_linetype_manual(values = setNames(c("dotted", "solid"), comparison)) +
  cowplot::theme_minimal_grid() +
  # cowplot::theme_cowplot()+
...
impulse_data <- moment_by_type %>% 
  ...
  group_by(subject, condition, period, variable, moment_type) %>% 
...
impulse_data %>% 
  ...
  geom_line(alpha=0.3, linewidth=0.3) +
  geom_point(alpha=0.5, size=1) + 
  facet_grid(variable~ condition + moment_type)+
  # scale_x_discrete(guide = guide_axis(n.dodge = 2))+
  cowplot::theme_minimal_grid()+
  theme(
        # axis.text.x = element_text(angle = 45, hjust = 1),
  ...
  labs(
    x = "Period",
    y = "PF Impulse (N.m.s/BW)",
    title = "Impulse Across Stance Phase by Period",
  ) +
  geom_text(stat = "summary", 
            fun = mean,
            aes(label = sprintf("%.2f", ..y..), 
