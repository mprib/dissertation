...
mean_across_stance %>% 
  ...
  labs(
    ...
    y = "Belt Speed (m/s)",
    title = "Actual Belt Speed Across Stance Phase By Participant",
    subtitle = "Averaged Across 5 Steps",
    color = "Condition"
  )+
  scale_color_manual(values= c("IPSI_BeltSpeed" = "blue", "CONTRA_BeltSpeed" = "red"))  # Added color specification
...
mean_across_stance %>% 
  filter(period %in% periods) %>% 
  mutate(period = factor(period,levels = periods)) %>% 
  filter(variable %in% c("PELVIS_COM") & axis == "Y") %>% view()
  group_by(period, condition, variable, normalized_time,stance_side) %>% 
  summarize(PF_torque = mean(value)) %>% 
  ggplot(aes(x=normalized_time,y= PF_torque, linetype=period, color=condition))+
  geom_line()+
